...
export const verifyPaystackPayment = async (req, res, next) => {
  ...
  if (!reference || !courseId) {
    return next(new HttpError('Missing reference or courseId', 400));
  }
  ...
  try {
    ...
    res.status(200).json({ success: true, message: "Payment verified & student enrolled" });
  } catch (err) {
    console.error(err.message);
    return next(new HttpError('An error occurred during verification', 500));
  }
};